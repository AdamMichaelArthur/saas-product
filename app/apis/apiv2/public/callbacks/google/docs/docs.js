...
export default class Docs extends Google {
  ...
  async getProfile(token){
    ...
    var oauth2Client = this.getOAuthClient("https://app.saas-product.com/v2.0/public/callbacks/google/docs/authorized");
  ...
  async authorized(){
    ...
    let docsIntegrationCollecton = this.database.db.collection("integrationsdocs");
    ...
    docsIntegrationCollecton.updateOne(
      { email: profile.email }, 
      { $set: document },
      { upsert: true });
