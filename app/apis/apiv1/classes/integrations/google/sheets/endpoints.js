...
var sheets = require("@classes/integrations/google/sheets/sheets.js")

class Sheets {
  constructor(req, res, next) {
    this.className = "sheets";
    this.req = req;
    this.res = res;
    this.next = next;
  ...
  error(err) {
     ...
     *
     *  URL Pattern To Access This Function Call
     *  ----------------------------------------
     *
     *  https://app.contentbounty.com/v1.0/api/google/sheets/createSpreadsheet
     *  http://localhost:3000/api/google/sheets/createSpreadsheet
     *
     *  Body
     *  ----
     *
    ...
    */
	  ...
	  async createSpreadsheet(){
	  	var sheets = await this.sheets.createSpreadsheet()
	  	this.output(sheets);
	  }
...
function next(req, res) {
  var defaultResponseObject = helpers.defaultResponseObject("sheets")
  defaultResponseObject["sheets"] = res.locals.response;
  res.status(200);
...
function routeDataSource(req, res, next) {
  ...
  var action = helpers.getParameter(fullUrl, "sheets");

  if (typeof action == 'undefined') {
    action = helpers.getParameter(fullUrl, "sheets");
  }

  var Action = new Sheets(req, res, next);
...
var methods = Object.getOwnPropertyNames(Sheets.prototype);
